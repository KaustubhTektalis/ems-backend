...
public class AuthServiceImpl implements AuthService {
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        if (request.getUsername().contains("@")) {
            ...
            user = userRepository
                    .findByEmployeeCompanyEmail(request.getUsername())
                    .orElseThrow(() ->
        ...
        List<String> roles =
                ...
                .stream()
                .map(r -> r.getRole().getRole().name())
                .toList();

        
        String accessToken =
                jwtUtils.generateAccessToken(user.getEmpId(), roles);

        String refreshToken =
                jwtUtils.generateRefreshToken(user.getEmpId());

        return LoginResponse.builder()
                .token(accessToken)
                .refreshToken(refreshToken)
                .empId(user.getEmpId())
                .companyEmail(user.getEmployee().getCompanyEmail())
                .roles(roles)
                .build();
    }
    ...
    public LoginResponse refreshToken(RefreshTokenRequest request) {
        ...
        String newAccessToken =
                jwtUtils.generateAccessToken(empId, roles);
